...
var schema = Joi.object().keys(***REMOVED***
    ...
    password: Joi.string().regex(/^[a-zA-Z0-9]***REMOVED***3,30***REMOVED***$/),
    access_token: [Joi.string(), Joi.number()],
    birthyear: Joi.number().integer().min(1900).max(2013),
    email: Joi.string().email()
***REMOVED***).with('username', 'birthyear').without('password', 'access_token');

Joi.validate(***REMOVED*** username: 'abc', birthyear: 1994 ***REMOVED***, schema, function (err, value) ***REMOVED*** ***REMOVED***);  // err === null -> valid
```
...
```javascript
Joi.validate(***REMOVED*** a: 'a string' ***REMOVED***, schema, function (err, value) ***REMOVED*** ***REMOVED***);
```
