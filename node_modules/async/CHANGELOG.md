...
- Async is also available as a collection of ES2015 modules in the new `async-es` package. (`import ***REMOVED***forEachSeries***REMOVED*** from 'async-es'`) (#984, #996)
...
- `***REMOVED***METHOD***REMOVED***` and `***REMOVED***METHOD***REMOVED***Series` are now implemented in terms of `***REMOVED***METHOD***REMOVED***Limit`. This is a major internal simplification, and is not expected to cause many problems, but it does subtly affect how functions execute internally. (#778, #847)
