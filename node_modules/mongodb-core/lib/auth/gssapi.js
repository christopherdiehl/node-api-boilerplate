...
GSSAPI.prototype.auth = function(server, connections, db, username, password, options, callback) ***REMOVED***
  ...
  while(connections.length > 0) ***REMOVED***
    ...
    var execute = function(connection) ***REMOVED***
      ...
      GSSAPIInitialize(self, db, username, password, db, gssapiServiceName, server, connection, options, function(err, r) ***REMOVED***
        // Adjust count
        count = count - 1;
...
//
// Initialize step
var GSSAPIInitialize = function(self, db, username, password, authdb, gssapiServiceName, server, connection, options, callback) ***REMOVED***
...
var MongoDBGSSAPIFirstStep = function(self, mongo_auth_process, payload, db, username, password, authdb, server, connection, callback) ***REMOVED***
  ...
  ***REMOVED***;
  ...
  server(connection, new Query(self.bson, "$external.$cmd", command, ***REMOVED***
...
//
// Perform first step against mongodb
var MongoDBGSSAPISecondStep = function(self, mongo_auth_process, payload, doc, db, username, password, authdb, server, connection, callback) ***REMOVED***
...
var MongoDBGSSAPIThirdStep = function(self, mongo_auth_process, payload, doc, db, username, password, authdb, server, connection, callback) ***REMOVED***
  // Build final command
  var command = ***REMOVED***
      saslContinue: 1
    , conversationId: doc.conversationId
...
GSSAPI.prototype.reauthenticate = function(server, connections, callback) ***REMOVED***
  ...
  for(var i = 0; i < authStore.length; i++) ***REMOVED***
    this.auth(server, connections, authStore[i].db, authStore[i].username, authStore[i].password, authStore[i].options, function(err) ***REMOVED***
      count = count - 1;
